...
class OpenSourceJudge(JudgeBase):
    def __init__(self, judge_name, max_n_tokens, judge_model):
        ...
        assert isinstance(
            self.judge_model, ray.actor.ActorHandle
        ), "Expect OpenSourceJudge Model to be a RayActor"
    ...
    def create_conv(self, full_prompt, behavior, context_str=None):
        ...
        if conv.sep2 is not None:
            ...
            trunc_len = -1
        return conv.get_prompt()[: trunc_len]
    ...
    def score(self, attack_prompt_list, target_response_list, behavior, context_str=None):
        ...
        generate_future = self.judge_model.batched_generate.remote(
            convs_list,
        ...
        raw_outputs = [
            self.init_msg + o for o in raw_outputs
        ]  # add back prefixes init to process_output
